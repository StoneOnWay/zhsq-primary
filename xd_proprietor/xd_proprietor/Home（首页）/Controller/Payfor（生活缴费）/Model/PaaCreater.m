...
static PaaCreater *paaCreat = nil;

+ (PaaCreater *)sharePaaCreater {
    if(paaCreat==nil){
        paaCreat=[[PaaCreater alloc] init];
    }
    return paaCreat;
}
...
//                         @"http://www", @"receiveUrl",
                         ...
                         //通联分配给商户的ID
                         @"008340142150000", @"merchantId",//测试商户号
                         
                         //商户当前支付订单号
                         [paaCreat payid], @"orderNo",
                         
                         //订单支付金额
                         //分做单位
                         //最小0.01元
                         [paaCreat AmountOfMoney], @"orderAmount",
...
//                         @"", @"cardNo",
                         //商户生成订单时间戳
                         //必须允许的订单时间范围内
                         [paaCreat createdate], @"orderDatetime",
                         
                         //商品名称
                         [paaCreat productName], @"productName",
                         
                         [NSString stringWithFormat:@"<USER>%@</USER>",[paaCreat userid]], @"ext1",
    ...
    NSArray *paaDic = @[
                        ...
                        //支付通知结果以此为准,后台通知商户网站支付结 果的 url 地址
                        //                         @"http://www", @"receiveUrl",
                        @"http://www.ququapp.com/p2pmoney/index.php?d=api&c=common_base&m=order_receive", @"receiveUrl",
...
//                                                 @"100020091218001", @"merchantId",
                        ...
                        //对于非外卡支付
                        //商户用户号在通联会员系统对应的会员号
//                        @"<USER>201406131006408</USER>", @"ext1",//若商户用户未注册通联会员号,该行不需要,可以注释掉
                        //此时为 非会员模式,只能走认证支付
