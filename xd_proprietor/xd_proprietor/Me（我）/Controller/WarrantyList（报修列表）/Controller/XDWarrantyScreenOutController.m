//
//  XDWarrantyScreenOutController.m
//  XD业主
//
...
@interface XDWarrantyScreenOutController ()<UITableViewDelegate,UITableViewDataSource>
...
//选择的工单类型id
@property (assign , nonatomic)NSInteger workOrderType;
@end

@implementation XDWarrantyScreenOutController
...
- (void)viewDidLoad {
    ...
    //加载工单类别
    [self loadWorkOrderType];
    
    
    
}
//加载工单类别
- (void)loadWorkOrderType {
    ...
    [[XDAPIManager sharedManager]requestWarrantyScreenTypeParameters:dic CompletionHandle:^(id responseObject, NSError *error) {
        ...
        if ([responseObject[@"resultCode"] isEqualToString:@"0"] ) {
            ...
            if (weakSelf.typePopArray.count != 0) {
                ...
                weakSelf.workOrderType = [weakSelf.typePopArray.firstObject[@"id"] integerValue];
            }
...
- (IBAction)screenOutBtn:(id)sender {
    ...
    if ([self.delegate respondsToSelector:@selector(XDWarrantyScreenOutControllerWithStartTime:endTime:workOrderType:)]) {
        [self.delegate XDWarrantyScreenOutControllerWithStartTime:self.startLabel.text endTime:self.endLabel.text workOrderType:self.workOrderType];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    self.workOrderType = [self.typePopArray[indexPath.row][@"id"] integerValue];
