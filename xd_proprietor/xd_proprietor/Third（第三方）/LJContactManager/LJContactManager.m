...
- (void)createNewContactWithPhoneNum:(NSString *)phoneNum controller:(UIViewController *)controller
{
    if (IOS9_OR_LATER)
    {
        ...
        [nav.navigationBar setBackgroundImage:[UIImage createImageWithColor:RGB(0, 144, 238)] forBarMetrics:UIBarMetricsDefault];
...
- (void)_presentFromController:(UIViewController *)controller
{
    if (IOS9_OR_LATER)
    {
        ...
        NSDictionary *textAt = @{NSFontAttributeName : CFont(19, 17),NSForegroundColorAttributeName : [UIColor blackColor]
                                 };
        ...
        [self _authorizationStatus:^(BOOL authorization) {
            if (authorization)
            {
                [controller presentViewController:pc animated:YES completion:^{
    ...
    else
    {
        ...
        [self _authorizationStatus:^(BOOL authorization) {
            ...
            if (authorization)
            {
                [controller presentViewController:pvc animated:YES completion:^{
                    [[UIApplication sharedApplication] setStatusBarStyle:UIStatusBarStyleDefault];
                }];
            }
            else
