...
-(void)setTitleFont:(NSInteger)titleTextFont {
...
-(UIView *)showView {
    if (!_showView) {
        ...
        CGFloat h = _titleItems.count * 50 + 60 + 50;
...
-(UIView *)bottomView {
    if (!_bottomView) {
        ...
        // 分割线
        UIView *lineH = [[UIView alloc] initWithFrame:CGRectMake(0, 0, w, 0.5)];
        lineH.backgroundColor = BianKuang;
        [_bottomView addSubview:lineH];
        UIView *lineV = [[UIView alloc] initWithFrame:CGRectMake(w / 2, 0, 0.5, h)];
        lineV.backgroundColor = BianKuang;
...
//        [_cacelBtn setTitleColor:textsColor forState:UIControlStateNormal];
...
//        [_confirmBtn setTitleColor:textsColor forState:UIControlStateNormal];
        _cacelBtn.titleLabel.font = [UIFont systemFontOfSize:15];
