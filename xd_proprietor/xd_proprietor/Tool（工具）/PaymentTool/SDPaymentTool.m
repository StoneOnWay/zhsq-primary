...
- (void)WechatPayWithMoney:(NSString *)money OrderName:(NSString*)ordername OrderNumber:(NSString *)orderNumber
{
    ...
    
    {
        ...
        // 价格 (微信是<分>为单位)
        float orderPrice = [money floatValue] ;
...
- (void)WechatPayWithParamer:(NSDictionary *)signParams {

    
    NSMutableString *stamp  = [signParams objectForKey:@"timestamp"];
    //调起微信支付
    PayReq *reqs             = [[PayReq alloc] init];
    reqs.openID              = [signParams objectForKey:@"appid"];
    reqs.partnerId           = [signParams objectForKey:@"partnerid"];
    reqs.prepayId            = [signParams objectForKey:@"prepayid"];
    reqs.nonceStr            = [signParams objectForKey:@"noncestr"];
    reqs.timeStamp           = stamp.intValue;
    reqs.package             = [signParams objectForKey:@"package"];
    reqs.sign                = [signParams objectForKey:@"sign"];
    [WXApi sendReq:reqs];
...
-(NSString*) createMd5Sign:(NSMutableDictionary*)dict
{
    ...
    //添加key字段
    [contentString appendFormat:@"key=%@", @"19caa17485a346778c6307ccdc70c9d6"];
    //得到MD5 sign签名
